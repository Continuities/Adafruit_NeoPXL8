...
#define LATCHTIME 50 ///< Time, in microseconds, for end-of-data latch
...
#define BIT_FREQUENCY 800000 // hz
#define CLOCK_PAD 1200000 // hz Not sure why this is what it is
...
boolean Adafruit_NeoPXL8::begin(void) {
  ...
  if (pixels) {               // Successful malloc of NeoPixel buffer?
    ...
    uint32_t bytesTotal = numLEDs * bytesPerPixel * 4 + EXTRASTARTBYTES + 3;
    if ((dmaBuf = (uint8_t *)malloc(bytesTotal))) {
      ...
      while (TCC0->SYNCBUSY.bit.CC0)
        ...
        // 2.4 GHz clock: 4 DMA xfers per NeoPixel bit = 400 KHz
#ifdef __SAMD51__
      TCC0->PER.reg = ((48000000 + CLOCK_PAD) / (4 * BIT_FREQUENCY)) - 1;
#else
      TCC0->PER.reg = ((F_CPU + CLOCK_PAD) / (4 * BIT_FREQUENCY)) - 1;
#endif
